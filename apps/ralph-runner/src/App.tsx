import { Component, createSignal, Show, createMemo, createEffect, onMount } from "solid-js"
import { Header, RepoInput, TaskInput, ProviderSelector, TimelinePanel, TerminalPanel, FileExplorer, ComparisonView, AgentRaceView, DemoAgentRace, RalphDashboard } from "./components"
import { useRun } from "./hooks"
import type { Provider, RepoMetadata } from "./types"

type ActiveTab = "timeline" | "terminal" | "files" | "comparison" | "race" | "demo" | "dashboard"
...
const App: Component = () => {
  ...
  createEffect(() => {
    console.log("[App] run.runId():", run.runId())
    console.log("[App] run.runState():", run.runState())
    console.log("[App] run.getEvents():", run.getEvents())
    console.log("[App] run.isConnected():", run.isConnected())
  })
  ...
  createEffect(() => {
    if (run.runId()) {
      // Use dashboard as default view when running
            ...
            <Show when={activeTab() !== "demo"}>
              <div class="flex-1 flex items-center justify-center text-[var(--color-text-muted)]">
                <div class="text-center">
                  ...
                  <p class="text-sm mb-4">{statusText()}</p>
                  <button
                    class="px-4 py-2 text-sm font-medium rounded-md bg-purple-600/20 text-purple-400 hover:bg-purple-600/30 border border-purple-500/30 transition-colors"
