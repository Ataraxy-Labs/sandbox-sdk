...
export async function createSandboxInDb(
  ...
  try {
    const id = await client.mutation(api.sandboxes.create, {
      userId,
      sandboxId,
      provider: provider as "modal" | "daytona" | "e2b" | "blaxel" | "cloudflare" | "vercel",
      repoUrl,
    })
...
export async function createRalphInDb(
  ...
  if (!client) return null
  try {
    const id = await client.mutation(api.ralphs.create, { userId, sandboxId, task })
...
export async function addAgentEvent(
  ...
  try {
    await client.mutation(api.agentEvents.add, {
      ralphId,
      type: type as "status" | "thought" | "tool_call" | "tool_result" | "output" | "error" | "complete" | "ralph_iteration" | "ralph_complete" | "opencode_ready" | "clone_progress" | "install_progress",
      data,
    })
  } catch (err) {
...
export async function writeSSEEventToConvex(
  ...
  try {
    await client.mutation(api.agentEvents.add, {
      ralphId,
      type: mappedType as "status" | "thought" | "tool_call" | "tool_result" | "output" | "error" | "complete" | "ralph_iteration" | "ralph_complete" | "opencode_ready" | "clone_progress" | "install_progress",
      data: {
