...
import { PROVIDER_OPTIONS, IMAGE_OPTIONS, runtimeFromImage, languageFromRuntime } from "@/types"
...
export default function Playground() {
  ...
  const containerReady = () => focusedContainer()?.status === "ready"

  return (
    <div class="h-full flex flex-col bg-[var(--color-bg)]">
      {/* Toolbar */}
      <div class="flex items-center justify-between h-11 px-4 border-b border-[var(--color-border)] bg-[var(--color-bg)] shrink-0">
        <div class="flex items-center gap-3">
          {/* Container status indicator */}
          <Show
            when={focusedContainer()}
            fallback={
              <div class="flex items-center gap-2 text-[var(--color-text-dim)]">
                <div class="w-1.5 h-1.5 rounded-full bg-[var(--color-text-dim)]" />
                <span class="text-[12px]">No container</span>
              </div>
            }
          >
            <div class="flex items-center gap-2">
              <div
                class={`w-1.5 h-1.5 rounded-full ${
                  focusedContainer()?.status === "ready"
                    ? "bg-[var(--color-success)] shadow-[var(--color-glow-success)]"
                    : focusedContainer()?.status === "creating"
                      ? "bg-[var(--color-warning)] animate-pulse"
                      : "bg-[var(--color-error)]"
                }`}
              />
              <span class={`text-[12px] font-medium ${
                focusedContainer()?.status === "ready" ? "text-[var(--color-success)]" :
                focusedContainer()?.status === "creating" ? "text-[var(--color-warning)]" :
                "text-[var(--color-error)]"
              }`}>{focusedContainer()?.status}</span>
            </div>

            <div class="w-px h-4 bg-[var(--color-border)]" />

            {/* Provider badge */}
            <div class="flex items-center gap-1.5 px-2 py-1 rounded-md bg-[var(--color-bg-tertiary)] border border-[var(--color-border)]">
              <span class="text-[11px] font-medium text-[var(--color-text-muted)]">{focusedContainer()?.provider}</span>
            </div>

            {/* Image */}
            <span class="text-[12px] text-[var(--color-text-dim)] font-mono">
              {focusedContainer()?.image}
            </span>

            {/* ID */}
            <Show when={focusedContainer()?.id}>
              <div class="flex items-center gap-1 text-[var(--color-text-dim)]">
                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
                </svg>
                <span class="text-[11px] font-mono">{focusedContainer()?.id?.slice(0, 8)}</span>
              </div>
            </Show>
          </Show>
        </div>
        ...
        <div class="flex items-center gap-3">
          {/* Provider selector */}
          <select
            class="bg-[var(--color-bg-tertiary)] text-[11px] px-2 py-1.5 rounded-md border border-[var(--color-border)] text-[var(--color-text)] focus:outline-none focus:border-[var(--color-accent)] transition-all cursor-pointer"
            value={state.createDraft.provider}
            onChange={(e) => setState("createDraft", "provider", e.currentTarget.value as Provider)}
          >
          ...
          <select
            class="bg-[var(--color-bg-tertiary)] text-[11px] px-2 py-1.5 rounded-md border border-[var(--color-border)] text-[var(--color-text)] focus:outline-none focus:border-[var(--color-accent)] transition-all cursor-pointer"
            value={state.createDraft.image}
            onChange={(e) => setState("createDraft", "image", e.currentTarget.value)}
          >
          ...
          {/* Run button */}
          <button
            class="flex items-center gap-2 h-8 px-4 text-[12px] font-semibold bg-white text-black rounded-lg hover:bg-gray-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            onClick={runCode}
            disabled={!containerReady() || focusedContainer()?.isRunning}
          >
            <Show
              when={!focusedContainer()?.isRunning}
              fallback={
            ...
            {focusedContainer()?.isRunning ? "Running" : "Run"}
          </button>
        </div>
      </div>
